...
from serializer import (build_computed_entry, build_seasonality_grid, build_annual_grid,
                          write_json_files, _nested_set, null_seasonality_grid, null_annual_grid)
...
def run(current_date: date = None) -> int:
    ...
    for r in resolved:
        if r.resolution == 'unresolved':
            ...
            #
            # dataStatus distinguishes WHY it's unresolved -- a category can be
            # unresolved because every candidate is genuinely below the 3yr floor
            # (r.note carries "under the floor"/"skip" language from sources.py in
            # that case) vs. a candidate that's old enough but confirmed broken on
            # the provider side (sources.py's own "CONFIRMED NON-FUNCTIONAL" notes,
            # e.g. strategy/Momentum's benchmark candidate). Both render as null
            # today with zero way to tell them apart; surfacing r.note's status
            # lets the frontend show "building history" vs "source needs fixing"
            # instead of one undifferentiated blank state for both.
            note = (r.note or '').lower()
            broken = 'non-functional' in note or 'confirmed' in note and 'non' in note
            _nested_set(computed, r.group, r.name, {
                ...
                'dataStatus': {
                    'status': 'source-broken' if broken else 'unresolved',
                    'note': r.note,
                },
            })
            continue
        try:
            ...
            # Previously: print(f"SKIP {r.group}/{r.name}: {e}") -- swallowed the
            # real cause (a bare exception message, often just "NoneType has no
            # attribute..." with zero indication of WHERE), and left the category
            # with NO computed.json entry at all rather than a labeled 'error'
            # state, which is indistinguishable from quality_guard's other
            # completeness failures.  Now: full traceback to the run log (so a
            # fetch-vs-cache-vs-formula bug is actually diagnosable from CI output),
            # AND a null-filled entry with dataStatus='error' so this category
            # doesn't silently vanish from computed.json and still passes
            # check_completeness -- but the frontend can now tell "genuine crash"
            # apart from "building history" or "context/currency, not scored."
            print(f"ERROR {r.group}/{r.name}: {e}")
            traceback.print_exc()
            _nested_set(computed, r.group, r.name, {
                'structure': None, 'fundamental': None, 'contra': None,
                'trend': None, 'cycle': None, 'vol': None, 'opportunity': None,
                ...
                'pivotDist': None, 'posPct': None, 'arrow': None,
                'accel': None, 'exhaustion': None,
                'dataStatus': {'status': 'error', 'note': str(e)},
            })
            # quality_guard.check_seasonality_grid_shape() requires a full 12x6
            # entry for every NON-unresolved category (this one resolved fine --
            # r.resolution is 'etf'/'benchmark'/etc, it just crashed downstream).
            # Without this, one broken category (Chemicals/Logistics/etc.) would
            # abort the entire deploy via a missing-seasonality-entry issue,
            # rather than degrading gracefully like its computed.json entry does.
            _nested_set(seasonality, r.group, r.name, null_seasonality_grid())
            _nested_set(annual, r.group, r.name, null_annual_grid())
