...
VND constituents to USD (VNM's own currency) before combining, then
converts the resulting USD composite to INR (fx.convert_usd_to_inr()) so
it's displayed on the same basis as every other Global Markets category.
Any FUTURE composite that mixes currencies would need the same treatment;
build_composite() still guards against being called directly on a
registered cross-currency name (CROSS_CURRENCY_COMPOSITES) so a new one
can't silently produce a wrong blended series before someone writes its
conversion path.
...
def build_vietnam_composite(constituent_prices: dict[str, pd.Series]) -> pd.DataFrame:
    ...
    series, and finally converts the resulting USD composite to INR so
    Vietnam is displayed on the same INR basis as every other Global
    Markets category (fx.convert_usd_to_inr(), using the same 'INR=X'
    series the broad-market USD/INR category and fx.convert_to_inr()'s
    bridge path already rely on).
    """
    converted = {
    ...
    usd_composite = build_composite(converted, category_name='')
    usd_composite['close'] = fx.convert_usd_to_inr(usd_composite['close'])
...
def get_category_series(resolved: ResolvedSource) -> pd.DataFrame:
    ...
    if resolved.resolution in ('etf', 'benchmark', 'futures', 'fx'):
        ...
        # Global Markets: fx.py's own docstring says Trend/Position/Cycle should
        # run on the INR-converted series ("what matters to an Indian investor
        # comparing against domestic opportunities"), but nothing was actually
        # calling fx.build_dual_series() -- every Global Markets score was being
        # computed on local-currency closes, so e.g. a Nikkei rally that's flat
        # in INR terms would still have scored bullish. Fixed here, the single
        # choke point every downstream factor module reads through.
        if resolved.group == 'global-markets' and resolved.name in fx.CURRENCY_MAP:
            converted, _method = fx.convert_ohlc_to_inr(cached, fx.CURRENCY_MAP[resolved.name])
            return converted
        return cached
