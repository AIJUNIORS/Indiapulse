...
CATEGORY_SOURCES: list[CategorySource] = [
    ...
    CategorySource(
        ...
        candidates={
            ...
            'benchmark': InstrumentCandidate(kind='benchmark', source_type_label='Index', history_years=0.0, symbol='NIFTY_LARGEMID250.NS', return_basis='TRI', verified=False, note='CONFIRMED NON-FUNCTIONAL: Yahoo chart API only returns todays date regardless of start date requested. Fund fallback 0P0001NQZ5.BO already covers this category, harmless.'),
        },
    ),
    ...
    CategorySource(
        ...
        candidates={
            ...
            'benchmark': InstrumentCandidate(kind='benchmark', source_type_label='Index', history_years=0.0, symbol='NIFTY_MICROCAP250.NS', return_basis='TRI', verified=False, note='CONFIRMED NON-FUNCTIONAL: same Yahoo chart API issue as LargeMidcap 250. Fund fallback 0P0001R64W.BO already covers this category, harmless.'),
        },
    ),
    ...
    CategorySource(
        ...
        candidates={
            ...
            'benchmark': InstrumentCandidate(kind='benchmark', source_type_label='Index', history_years=0.0, symbol='NIFTY200MOMENTM30.NS', return_basis='TRI', verified=False, note='CONFIRMED NON-FUNCTIONAL: same Yahoo chart API issue. NO working fallback yet -- MOMOMENTUM.NS etf is only 1.4y, under the 3y floor. Category will correctly SKIP until that ages past 3y or a better source is found.'),
            'etf_alt': InstrumentCandidate(kind='etf', source_type_label='ETF', history_years=0.0, symbol='HDFCMOMENT.NS', return_basis='TRI', verified=False, note='UNCONFIRMED -- HDFC Nifty200 Momentum 30 ETF, surfaced as a possible older-vintage alternative to MOMOMENTUM.NS (same underlying index, different AMC/launch date). history_years is a placeholder (0.0) until check_tickers.py actually confirms row count and first-trade date against live Yahoo data -- do NOT flip verified=True or set a real history_years off this note alone. If it clears the 3.0y floor once verified, resolve_source() should prefer this over the confirmed-broken benchmark.'),
        },
    ),
    CategorySource(
    ...
    CategorySource(
        ...
        candidates={
            # CORRECTED history_years -- CHEMICAL.NS is confirmed (HDFC Securities,
            # IndMoney) to be the Kotak Nifty Chemicals ETF's real NSE ticker; it is
            # NOT a broken/wrong symbol, and there is no separate "KOTH.NS" ticker --
            # that name doesn't resolve to anything on NSE (checked directly). The
            # actual bug was the history_years value: the fund launched 2025-11-12,
            # so as of today it has ~0.75y of history, not the 3.1y previously
            # hardcoded here. That stale number was letting a 9-month-old fund
