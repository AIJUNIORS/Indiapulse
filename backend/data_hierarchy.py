...
def resolve_source(cat: CategorySource) -> ResolvedSource:
    ...
    # 'etf_alt' -- a second, usually-unverified ETF candidate for categories
    # where the primary etf is under the floor and a plausible alternative
    # exists (e.g. a different AMC's fund tracking the same index, older
    # vintage). Only reached if 'etf' itself didn't clear the floor above.
    # Same MIN_HISTORY_YEARS gate applies -- an unverified candidate with
    # history_years still at its 0.0 placeholder (see sources.py's own note
    # convention) will correctly keep falling through to benchmark/composite
    # below until someone runs check_tickers.py and fills in a real number.
    etf_alt = cat.candidates.get('etf_alt')
    if etf_alt and etf_alt.history_years >= MIN_HISTORY_YEARS:
        return ResolvedSource(
            group=cat.group, name=cat.name, resolution='etf',
            symbol=etf_alt.symbol, display_etf=None, constituents=None,
            history_years=etf_alt.history_years, return_basis=etf_alt.return_basis,
            confidence='high', flag=cat.flag, verified=etf_alt.verified, note=etf_alt.note,
        )
